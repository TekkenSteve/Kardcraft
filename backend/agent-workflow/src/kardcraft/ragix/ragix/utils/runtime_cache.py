...
- Component configs (parsers, processors, models, etc.)
- Object instances (with pickle serialization)
- Metadata and configuration data
- Temporary data with TTL

All caches use explicit invalidation on data changes, with TTL as safety net.
Uses orjson for fast JSON serialization when available.
"""
import json
import time
...
#     from ..config.suna_config import SUNA_CONFIG
#     from ..config.config_helper import _extract_agentpress_tools_for_run
...
#     try:
#         from ..agents import repo as agents_repo
#         from ..agents.agent_loader import get_agent_loader
        
#         agent_ids = await agents_repo.get_user_agent_ids(user_id)
        
#         if not agent_ids:
#             try:
#                 from ..utils.ensure_suna import ensure_suna_installed
...
#     try:
#         from ..agents.pipeline.slot_manager import invalidate_tier_cache
...
# ============================================================================
# PARSER CONFIG CACHE - 解析器配置缓存，避免序列化复杂实例
# ============================================================================
PARSER_CONFIG_TTL = 3600  # 1小时

def _get_parser_config_key(cache_key: str) -> str:
    """Generate Redis cache key for parser config."""
    return f"parser_config:{cache_key}"


async def get_cached_parser_config(cache_key: str) -> Optional[Dict[str, Any]]:
    """
    Get parser configuration from Redis cache.
    Returns config dict that can be used to recreate parser instance.
    """
    redis_key = _get_parser_config_key(cache_key)
    ...
    try:
        ...
        if cached:
            data = _json_loads(cached) if isinstance(cached, (str, bytes)) else cached
            logger.debug(f"⚡ Redis cache hit for parser config: {cache_key}")
            return data
    except Exception as e:
        logger.warning(f"Failed to get parser config from cache: {e}")
    
    return None


async def set_cached_parser_config(cache_key: str, config_data: Dict[str, Any]) -> None:
    """Cache parser configuration in Redis."""
    redis_key = _get_parser_config_key(cache_key)
    
    try:
        from ..services import redis as redis_service
        await redis_service.set(redis_key, _json_dumps(config_data), ex=PARSER_CONFIG_TTL)
        logger.debug(f"✅ Cached parser config in Redis: {cache_key}")
    except Exception as e:
        logger.warning(f"Failed to cache parser config: {e}")


async def invalidate_parser_config_cache(cache_key: str) -> None:
    """Invalidate cached parser configuration."""
    try:
        from ..services import redis as redis_service
        await redis_service.delete(_get_parser_config_key(cache_key))
        logger.debug(f"🗑️ Invalidated parser config cache: {cache_key}")
    except Exception as e:
        logger.warning(f"Failed to invalidate parser config cache: {e}")
