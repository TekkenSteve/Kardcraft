...
class RedisClient:
    ...
    async def get_client(self) -> Redis:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                stream_ping_result = self._stream_client.ping()
                if asyncio.iscoroutine(stream_ping_result):
                    await asyncio.wait_for(stream_ping_result, timeout=5.0)
    ...
    async def delete_multiple(self, keys: List[str], timeout: Optional[float] = None) -> int:
        ...
        try:
            ...
            deleted_count = sum(1 for r in (results or []) if r)
    ...
    async def incr(self, key: str, timeout: Optional[float] = None) -> int:
    ...
    async def decr(self, key: str, timeout: Optional[float] = None) -> int:
    ...
    async def sadd(self, key: str, *members, timeout: Optional[float] = None) -> int:
    ...
    async def srem(self, key: str, *members, timeout: Optional[float] = None) -> int:
    ...
    async def smembers(self, key: str, timeout: Optional[float] = None) -> _builtin_set:
    ...
    async def expire(self, key: str, seconds: int, timeout: Optional[float] = None) -> bool:
    ...
    async def ttl(self, key: str, timeout: Optional[float] = None) -> int:
    ...
    async def scan_keys(self, pattern: str, count: int = 100, timeout: Optional[float] = None) -> List[str]:
    ...
    async def scard(self, key: str, timeout: Optional[float] = None) -> int:
    ...
    async def zrangebyscore(self, key: str, min: str, max: str, timeout: Optional[float] = None) -> List[str]:
    ...
    async def zscore(self, key: str, member: str, timeout: Optional[float] = None) -> Optional[float]:
    ...
    async def llen(self, key: str, timeout: Optional[float] = None) -> int:
    ...
    async def lpush(self, key: str, *values, timeout: Optional[float] = None) -> int:
        timeout = timeout or DEFAULT_OP_TIMEOUT
        client = await self.get_client()
        result = await self._with_timeout(
    ...
    async def ltrim(self, key: str, start: int, end: int, timeout: Optional[float] = None) -> bool:
        timeout = timeout or DEFAULT_OP_TIMEOUT
        client = await self.get_client()
        result = await self._with_timeout(
    ...
    async def lrange(self, key: str, start: int, end: int, timeout: Optional[float] = None) -> list:
        timeout = timeout or DEFAULT_OP_TIMEOUT
        client = await self.get_client()
        result = await self._with_timeout(
    ...
    async def stream_add(self, stream_key: str, fields: Dict[str, str], maxlen: Optional[int] = None, 
    ...
    async def stream_read(self, stream_key: str, last_id: str = "0", block_ms: Optional[int] = None,
    ...
    async def xadd(self, stream_key: str, fields: Dict[str, str], maxlen: Optional[int] = None, 
    ...
    async def publish(self, channel: str, message: str, timeout: Optional[float] = None) -> int:
        """Publish message to Redis Pub/Sub channel with timeout protection."""
        timeout = timeout or DEFAULT_OP_TIMEOUT
        client = await self.get_client()
        result = await self._with_timeout(
            client.publish(channel, message),
            timeout_seconds=timeout,
            operation_name=f"publish({channel})",
            default=0
        )
        return result or 0
    ...
    async def xreadgroup(self, groupname: str, consumername: str, streams: Dict[str, str],
                         block: Optional[int] = None, count: Optional[int] = None, timeout: Optional[float] = None) -> List:
...
async def get(key: str, timeout: Optional[float] = None):
    return await redis.get(key, timeout=timeout)

async def set(key: str, value: str, ex: Optional[int] = None, nx: bool = False, timeout: Optional[float] = None):
    return await redis.set(key, value, ex=ex, nx=nx, timeout=timeout)

async def setex(key: str, seconds: int, value: str, timeout: Optional[float] = None):
    return await redis.setex(key, seconds, value, timeout=timeout)

async def delete(key: str, timeout: Optional[float] = None):
    return await redis.delete(key, timeout=timeout)

async def delete_multiple(keys: List[str], timeout: Optional[float] = None) -> int:
    return await redis.delete_multiple(keys, timeout=timeout)

async def incr(key: str, timeout: Optional[float] = None) -> int:
    return await redis.incr(key, timeout=timeout)

async def decr(key: str, timeout: Optional[float] = None) -> int:
    return await redis.decr(key, timeout=timeout)

async def sadd(key: str, *members, timeout: Optional[float] = None) -> int:
    return await redis.sadd(key, *members, timeout=timeout)

async def srem(key: str, *members, timeout: Optional[float] = None) -> int:
    return await redis.srem(key, *members, timeout=timeout)

async def smembers(key: str, timeout: Optional[float] = None) -> _builtin_set:
    return await redis.smembers(key, timeout=timeout)

async def expire(key: str, seconds: int, timeout: Optional[float] = None):
    return await redis.expire(key, seconds, timeout=timeout)

async def ttl(key: str, timeout: Optional[float] = None) -> int:
    return await redis.ttl(key, timeout=timeout)

async def scan_keys(pattern: str, count: int = 100, timeout: Optional[float] = None):
    return await redis.scan_keys(pattern, count=count, timeout=timeout)

async def scard(key: str, timeout: Optional[float] = None) -> int:
    return await redis.scard(key, timeout=timeout)

async def zrangebyscore(key: str, min: str, max: str, timeout: Optional[float] = None):
    return await redis.zrangebyscore(key, min=min, max=max, timeout=timeout)

async def zscore(key: str, member: str, timeout: Optional[float] = None):
    return await redis.zscore(key, member, timeout=timeout)

async def llen(key: str, timeout: Optional[float] = None) -> int:
    return await redis.llen(key, timeout=timeout)

async def lpush(key: str, *values, timeout: Optional[float] = None) -> int:
    return await redis.lpush(key, *values, timeout=timeout)

async def ltrim(key: str, start: int, end: int, timeout: Optional[float] = None) -> bool:
    return await redis.ltrim(key, start, end, timeout=timeout)

async def lrange(key: str, start: int, end: int, timeout: Optional[float] = None) -> list:
    return await redis.lrange(key, start, end, timeout=timeout)

async def stream_add(stream_key: str, fields: dict, maxlen: Optional[int] = None, approximate: bool = True, 
...
async def stream_read(stream_key: str, last_id: str = "0", block_ms: Optional[int] = None, 
...
async def xadd(stream_key: str, fields: dict, maxlen: Optional[int] = None, approximate: bool = True, 
               timeout: Optional[float] = None) -> Optional[str]:
    return await redis.xadd(stream_key, fields, maxlen=maxlen, approximate=approximate, timeout=timeout)

async def xread(streams: dict, count: Optional[int] = None, block: Optional[int] = None, timeout: Optional[float] = None):
...
async def xreadgroup(groupname: str, consumername: str, streams: Dict[str, str], 
                     block: Optional[int] = None, count: Optional[int] = None, timeout: Optional[float] = None):
