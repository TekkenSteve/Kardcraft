...
import (
	...
	v1support "task-orchestrator/internal/controller/restapi/v1/support"
...
func NewCardTemplatePreviewHandler(deps TemplatesDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		reqTemplateID := strings.TrimSpace(v1support.StringOrDefault(payload["template_id"], ""))
		reqVersion := v1support.IntOrDefault(payload["version"], 0)
		reqFront := v1support.StringOrDefault(payload["front_html"], "")
		reqBack := v1support.StringOrDefault(payload["back_html"], "")
		reqCSS := v1support.StringOrDefault(payload["css"], "")
		reqJS := v1support.StringOrDefault(payload["js"], "")
		...
		if reqTemplateID != "" {
			...
			availableProfiles, defaultProfile = v1support.ExtractTemplateProfiles(tpl.MappingSpec)
			if reqFront == "" {
		...
		proxyPayload := map[string]any{
			...
			"template_profile": v1support.StringOrDefault(payload["template_profile"], ""),
			"card_type":        v1support.StringOrDefault(payload["card_type"], ""),
			"preview_mode":     v1support.StringOrDefault(payload["preview_mode"], "high_fidelity"),
			"render_target":    v1support.StringOrDefault(payload["render_target"], "anki"),
			"mapping_spec":     payload["mapping_spec"],
		}
		if len(templateMappingSpec) > 0 {
			proxyPayload["mapping_spec"] = templateMappingSpec
		}
		if strings.TrimSpace(v1support.StringOrDefault(payload["template_profile"], "")) == "" && defaultProfile != "" {
			proxyPayload["template_profile"] = defaultProfile
		}
		if sample, ok := v1support.ResolveSampleFieldsFromMapping(templateMappingSpec, strings.TrimSpace(v1support.StringOrDefault(proxyPayload["template_profile"], ""))); ok {
			if _, exists := proxyPayload["sample_fields"]; !exists || v1support.MapFromAny(proxyPayload["sample_fields"]) == nil {
				proxyPayload["sample_fields"] = sample
			}
		}
		body, statusCode, err := v1support.ProxyJSON(r.Context(), deps.HTTPClient, deps.AnkiRuntimeURL, "/internal/anki/preview", proxyPayload)
		if err != nil {
		...
		if _, ok := previewResp["available_profiles"]; !ok || len(v1support.StringSliceFromAny(previewResp["available_profiles"])) == 0 {
			if len(availableProfiles) > 0 {
				previewResp["available_profiles"] = availableProfiles
			}
		}
		if profile := strings.TrimSpace(v1support.StringOrDefault(previewResp["template_profile"], "")); profile == "" {
			if selected := strings.TrimSpace(v1support.StringOrDefault(proxyPayload["template_profile"], "")); selected != "" {
...
func NewCardTemplateBuildApkgHandler(deps TemplatesDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		body, statusCode, err := v1support.ProxyJSON(r.Context(), deps.HTTPClient, deps.AnkiRuntimeURL, "/internal/anki/build-apkg", payload)
		if err != nil {
...
func templateProxyWithTemplateFallback(path string, deps TemplatesDeps, withJS bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		templateID := strings.TrimSpace(v1support.StringOrDefault(payload["template_id"], ""))
		if templateID != "" {
			if deps.ReadModel == nil {
				http.Error(w, "storage unavailable", http.StatusServiceUnavailable)
				return
			...
			if v1support.StringOrDefault(payload["front_html"], "") == "" {
				payload["front_html"] = tpl.FrontHTML
			}
			if v1support.StringOrDefault(payload["back_html"], "") == "" {
				payload["back_html"] = tpl.BackHTML
			}
			if v1support.StringOrDefault(payload["css"], "") == "" {
				payload["css"] = tpl.CSS
			}
			if withJS {
				if _, ok := payload["js"]; !ok || v1support.StringOrDefault(payload["js"], "") == "" {
					payload["js"] = tpl.JS
				}
				payload["version"] = tpl.LatestVersion
			}
		}
		if strings.TrimSpace(v1support.StringOrDefault(payload["front_html"], "")) == "" || strings.TrimSpace(v1support.StringOrDefault(payload["back_html"], "")) == "" {
			http.Error(w, "front_html and back_html are required", http.StatusBadRequest)
			return
		}
		body, statusCode, err := v1support.ProxyJSON(r.Context(), deps.HTTPClient, deps.AnkiRuntimeURL, path, payload)
		if err != nil {
