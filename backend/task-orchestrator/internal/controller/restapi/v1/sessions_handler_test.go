...
import (
	...
	v1handlers "task-orchestrator/internal/controller/restapi/v1/handlers"
...
func TestNormalizeWorkspaceResponse_StableAcrossRepeatedCalls(t *testing.T) {
	...
	first := v1handlers.NormalizeWorkspaceResponse(raw, "s1")
	second := v1handlers.NormalizeWorkspaceResponse(raw, "s1")
...
func TestExtractResultMessage_FromCanonicalOutcome(t *testing.T) {
	...
	message := v1handlers.ExtractResultMessage(outcome.ToMap())
...
func TestExtractResultMessage_FromJSONStringEnvelope(t *testing.T) {
	...
	message := v1handlers.ExtractResultMessage(raw)
...
func TestResolveSessionActiveTask_PicksMostRecentActive(t *testing.T) {
	...
	active, ok := v1handlers.ResolveSessionActiveTask(tasks)
	if !ok {
...
func TestResolveSessionActiveTask_NoActive(t *testing.T) {
	...
	_, ok := v1handlers.ResolveSessionActiveTask(tasks)
	if ok {
...
func TestControlStateFromTaskState(t *testing.T) {
	if got := v1handlers.ControlStateFromTaskState("RUNNING"); got != "ACTIVE_RUNNING" {
		t.Fatalf("expected ACTIVE_RUNNING, got %s", got)
	}
	if got := v1handlers.ControlStateFromTaskState("PAUSED"); got != "ACTIVE_PAUSED" {
		t.Fatalf("expected ACTIVE_PAUSED, got %s", got)
	}
	if got := v1handlers.ControlStateFromTaskState("CANCELED"); got != "IDLE" {
...
func TestIsTaskActiveStatusSet(t *testing.T) {
	...
	for _, status := range active {
		if !v1handlers.IsTaskActiveStatus(status) {
