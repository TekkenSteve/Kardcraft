...
type TaskOutcomeCard struct {
	...
	SuggestedQuestionType string   `json:"suggested_question_type,omitempty"`
...
func (o TaskOutcome) ToMap() map[string]any {
	...
	for _, card := range o.FinalCards {
		...
		if suggested := strings.TrimSpace(card.SuggestedQuestionType); suggested != "" {
			c["suggested_question_type"] = suggested
		}
...
func normalizeOutcomeCards(raw []any) []TaskOutcomeCard {
	...
	for _, item := range raw {
		...
		suggestedQuestionType := strings.TrimSpace(asString(card["suggested_question_type"]))
		...
		out = append(out, TaskOutcomeCard{
			...
			SuggestedQuestionType: firstNonEmpty(suggestedQuestionType, model, "mcq"),
