...
type InMemoryTaskRepository struct {
	mu       sync.RWMutex
	store    map[string]entity.TaskSnapshot
	inserted []string
}

func NewInMemoryTaskRepository() *InMemoryTaskRepository {
	return &InMemoryTaskRepository{store: make(map[string]entity.TaskSnapshot)}
}

func (r *InMemoryTaskRepository) GetByID(_ context.Context, id entity.TaskID) (*entity.Task, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	return entity.RestoreTask(snapshot), nil
}

func (r *InMemoryTaskRepository) Save(_ context.Context, aggregate *entity.Task) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *InMemoryTaskRepository) List(_ context.Context, filter repo.TaskFilter) ([]*entity.Task, int, error) {
	...
	out := make([]*entity.Task, 0, len(selected))
	for _, key := range selected {
		out = append(out, entity.RestoreTask(r.store[key]))
	}
	return out, total, nil
}

type EventHook func(events []entity.DomainEvent)
...
func (p *InMemoryEventPublisher) Publish(_ context.Context, events []entity.DomainEvent) error {
	if len(events) == 0 {
		return nil
	}
	p.mu.Lock()
...
func (p *InMemoryEventPublisher) Events() []entity.DomainEvent {
	p.mu.Lock()
	defer p.mu.Unlock()
	out := make([]entity.DomainEvent, len(p.events))
	copy(out, p.events)
	return out
}
